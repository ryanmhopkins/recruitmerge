  ...
  return (
    <main className="dashboard-page">
      <nav className="dashboard-nav">
        <Brand />
        <div className="dashboard-account"><span className={`plan-pill ${isPro ? 'pro' : ''}`}>{isPro ? 'Pro' : 'Free'}</span><button className={`button billing-button ${isPro ? 'secondary' : 'primary'}`} disabled={billingBusy} onClick={() => openBilling(isPro ? 'portal' : 'checkout')}>{billingBusy ? 'Opening…' : isPro ? 'Manage plan' : 'Upgrade to Pro'}</button><span className="account-avatar">{user?.email?.slice(0, 1).toUpperCase()}</span><span className="muted user-email">{user?.email}</span><button className="link-button" onClick={signOut}>Sign out</button></div>
      </nav>
